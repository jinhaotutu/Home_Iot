...
ONETNET_INFO oneNetInfo = {"22957375", "GVr8xtBda3c5WMxBwx=cluDQPxQ=",
							"183.230.40.39", "876",
							1,
							NULL, 0, 0, 0, 1, 0};
...
void OneNet_DevLink(const char* devid, const char* auth_key)
{
	...
	
	{
		...
		{
            if (EDP_UnPacketRecv(oneNetInfo.cmd_ptr) == CONNRESP)
            {
...
unsigned char OneNet_SendData(FORMAT_TYPE type, char *devid, char *apikey, DATA_STREAM *streamArray, unsigned short streamArrayCnt)
{
	...
	{
		OneNet_SendData_Picture(devid, (char *)camera_buffer, PIC_WIDTH*PIC_HEIGHT*2+54);
